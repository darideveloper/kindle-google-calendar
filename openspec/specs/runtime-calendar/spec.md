...
Define the request-time rendering of the weekly calendar page: the rolling 7-day window computed per request, runtime calendar configuration via `CALENDARS`, short-TTL caching of fetched ICS data, graceful handling of feed failures, and the freshness marker in the footer.
